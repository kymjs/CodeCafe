...
import com.kymjs.kjcore.utils.FileUtils;
...
public class DiskCache implements Cache {
    ...
    public synchronized Entry get(String key) {
        ...
        try {
            ...
            KJLoger.debug(String.format("%s: %s", file.getAbsolutePath(), e.toString()));
            remove(key);
            return null;
        } finally {
            if (cis != null) {
    ...
    public synchronized void initialize() {
        if (!mRootDirectory.exists()) {
            if (!mRootDirectory.mkdirs()) {
                KJLoger.debug(String.format("Unable to create cache dir %s",
    ...
    public synchronized void put(String key, Entry entry) {
        ...
        try {
            ...
            if (!success) {
                KJLoger.debug(String.format("Failed to write header for %s",
        ...
        if (!deleted) {
            KJLoger.debug(String.format("Could not clean up file %s", file.getAbsolutePath()));
        }
    }
    ...
    public synchronized void remove(String key) {
        ...
        removeEntry(key);
        if (!deleted) {
            KJLoger.debug(String.format("Could not delete cache entry for key=%s, filename=%s",
    ...
    private void pruneIfNeeded(int neededSpace) {
        ...
        if (HttpConfig.DEBUG) {
            KJLoger.debug(String.format("pruned %d files, %d bytes, %d ms", prunedFiles,
